...
async function fetchWikidata(code: string): Promise<WikiResult | null> {
  ...
  try {
    return await cachedFetchJson<WikiResult>(
      ...
      async () => {
        try {
          const sparql = `SELECT ?headLabel ?officeLabel WHERE { ?country wdt:P297 "${code}". ?country p:P35 ?stmt. ?stmt ps:P35 ?head. FILTER NOT EXISTS { ?stmt pq:P582 ?end } OPTIONAL { ?stmt pq:P1039 ?office } SERVICE wikibase:label { bd:serviceParam wikibase:language "en" } } LIMIT 1`;
